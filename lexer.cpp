   ...
   return;
    ...
    while(token!=';'&&token!='}'){
        ...
        if(token!=Id){   //错误声明
            ...
            execute_result+=":bad global declaration\n";
            token=-1;return;
        }
        if(cur_id[Class]){  //重复声明
            printf("%d\n",token);
            execute_result+=std::to_string(((char)(token)));

            printf("%d :duplicate global declaration\n",line);
            execute_result+=std::to_string(line)+": duplicate global declaration\n";
            is_exec_error=1;
            token=-1;return;
        }
...
void deal_enmu_declaration(){
   ...
   while(token!='}'){
       if(token!=Id){
           ...
           execute_result+=std::to_string(line)+":bad enum identifier"+std::to_string(((char)(token)))+" \n";
           is_exec_error=1;
           token=-1;return;
       }
...
void deal_function_declaration(){
    ...
    while(token!=')'){         //每次while匹配一个参数
        ...
        if(token!=Id){
            ...
            execute_result+=std::to_string(line)+": bad parameter declaration\n";
            is_exec_error=1;
            token=-1;return;
        }
        if (cur_id[Class] == Loc) {  
            printf("%d: duplicate parameter declaration\n", line);
            execute_result+=std::to_string(line)+": duplicate parameter declaration\n";
            is_exec_error=1;
            token=-1;return;
        }
       token_match(Id);
    ...
    while(token==Int||token==Char){   //变量定义
        ...
        while(token!=';'){  
            ...
            if(token!=Id){   //错误声明
                ...
                execute_result+=std::to_string(line)+":bad local declaration\n";
                is_exec_error=1;
                token=-1;return;
            }
            if(cur_id[Class]==Loc){  //重复声明 
                ...
                execute_result+=std::to_string(line)+":duplicate local declaration\n";
                is_exec_error=1;
                token=-1;return;
            }
            token_match(Id);   
...
void deal_expression(int level) {
    ...
    int *addr;
    {
        if (!token) {
            ...
            execute_result+=std::to_string(line)+":unexpected token EOF of expression\n";
            is_exec_error=1;
            token=-1;return;
        }
        if (token == Num) {
            ...
            *++text = (expr_type == CHAR) ? sizeof(char) : sizeof(int);
            ...
            if (token == '(') {
                ...
                if (id[Class] == Sys) {
                    ...
                    *++text = id[Value];
                    ...
                    execute_result+=std::to_string(line)+":bad function call\n";
                    is_exec_error=1;
                    token=-1;return;
                }
                ...
                *++text = id[Value];
                ...
                if (id[Class] == Loc) {
                    ...
                    *++text = id[Value];
                    ...
                    execute_result+=std::to_string(line)+":undefined variable\n";
                    is_exec_error=1;
                    token=-1;return;
                }
            ...
            if (expr_type >= PTR) {
                ...
                execute_result+=std::to_string(line)+":bad dereference\n";
                is_exec_error=1;
                token=-1;return;
            }
            ...
            *++text = (expr_type == CHAR) ? LC : LI;
            ...
            if (*text == LC || *text == LI) {
                ...
                execute_result+=std::to_string(line)+": bad address of\n";
                is_exec_error=1;
                token=-1;return;
            }
            ...
            *++text = XOR;
            ...
            if (*text == LC) {
                ...
                *++text = LI;
                ...
                execute_result+=std::to_string(line)+": bad lvalue of pre-increment\n";
                is_exec_error=1;
                token=-1;return;
            }
            *++text = PUSH;
            ...
            *++text = (expr_type == CHAR) ? SC : SI;
            ...
            execute_result+=std::to_string(line)+": baddeal_expression\n";
            is_exec_error=1;
            ...
            if (token == Assign) {
                ...
                if (*text == LC || *text == LI) {
                    *text = PUSH; // save the lvalue's pointer
                    ...
                    execute_result+=std::to_string(line)+": bad lvalue in assignment\n";
                    is_exec_error=1;
                    token=-1;return;
                }
                deal_expression(Assign);
                ...
                *++text = JZ;
                ...
                if (token == ':') {
                    ...
                    execute_result+=std::to_string(line)+":missing colon in conditional\n";
                    is_exec_error=1;
                    token=-1;return;
                }
                *addr = (int)(text + 3);
                ...
                *++text = MOD;
                ...
                if (*text == LI) {
                    ...
                    *++text = LC;
                    ...
                    execute_result+=std::to_string(line)+": bad value in increment\n";
                    is_exec_error=1;
                    token=-1;return;
                }
                ...
                *++text = PUSH;
                ...
                if (tmp > PTR) {
                    ...
                    *++text = MUL;
                    ...
                    execute_result+=std::to_string(line)+": pointer type expected\n";
                    is_exec_error=1;
                    token=-1;return;
                }
                expr_type = tmp - PTR;
                ...
                *++text = (expr_type == CHAR) ? LC : LI;
                ...
                execute_result+=std::to_string(line)+": compiler error, token = "+std::to_string(((char)(token)));
                token=-1;return;
            }
        }
    }
